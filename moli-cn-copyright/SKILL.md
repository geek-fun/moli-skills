...
每次生成创建一个版本目录，`修订历史.md` 跟踪变化。后续如需修正（如修改页眉、调整章节），重新运行并选择修订模式。
...
│   ├── 草稿/                        ← AI 中间产物，可删
│   └── 验证报告.md
├── copyright-v2/                    ← 修订版（修正问题后）
│   ├── 正式资料/
│   ├── 草稿/
│   └── 验证报告.md
├── 截图/                            ← 所有版本共享
├── 修订历史.md                      ← 自动记录每次变更
└── copyright-latest -> copyright-v2  ← 指向最新版本
```

**操作手册内的「文档修改记录」表格**自动与修订历史同步。

目录规范：所有墨吏生成的材料统一放在 `docs/moli/` 下，以 `<功能>-v<版本>` 命名，方便项目管理和 `.gitignore`。
