...
- 字体：宋体（Songti/SimSun）
- 字号：五号（10.5pt）
- 行间距：计算使 50 行恰好填满 A4 可用高度（约 4.7-4.8mm/行），避免底部大片空白
- 颜色：黑色（禁止彩色）
- 编码：UTF-8
...
```
第一优先：入口文件（main.ts, App.vue, pages.json, manifest.json）
第二优先：页面文件（按重要程度：主页→核心功能页→设置页→辅助页）
第三优先：核心业务组件
第四优先：API 接口封装
第五优先：工具函数
```
...
// ========== src/App.vue ==========
```
路径标记不计入代码行数。标记文件来源，让审核人员一眼识别代码归属。
...
- 正文行距：1.5 倍行距
- 段落缩进：首行缩进 2 字符
- 标题字体：黑体（Heiti/SimHei），颜色黑色
- 一级标题：黑体 三号(16pt) 加粗，黑色
- 二级标题：黑体 四号(14pt) 加粗，黑色
- 页眉：与源代码页眉一致（软件全称 + 版本号），字体宋体 9pt 黑色
- 页眉页码：**必须使用 Word 域代码** `PAGE` / `NUMPAGES`，禁止手写页码
- 页眉分隔线：页眉下方有分隔线
- 颜色：全文黑色（正文、标题、表格文字全部为黑色，禁止彩色文字）
- 目录：**必须使用 Word TOC 域代码**（`TOC \o "1-3" \h \z \u`），禁止手写目录列表
