...
| 总页数正好 60 页（前30+后30），不足60页全部提交 | 《登记办法》第十条 |
| 每页 ≥ 50 行有效代码，无空行无注释 | 《登记办法》第十条 |
| 最后一页以完整模块结束（如 `};`），最后页≥15行 | SSIA 申报指南 |
| 页眉包含软件全称 + 版本号 | 版权中心要求 |
| 页眉禁止出现公司名称、URL | 版权中心要求 |
| 页码必须连续 | 版权中心要求 |
| 代码来自真实项目，禁止 AI 编造 | 2026 新政 |
| 字节序 UTF-8（避免中文乱码） | 技术规范 |

#### 质量建议（不强制，但做了更好）

| 建议 | 理由 |
|---|---|
| 代码按入口文件→页面→组件→API→工具函数排序 | 审核员第一眼能看懂项目结构 |
| 每个文件前加标记：`// ========== path/file.js ==========` | 标记不计入行数，方便追溯 |
| 字体建议宋体 10.5pt（五号）或等宽字体 | 清晰可读即可 |
| 行高可计算使 50 行填满 A4（约 4.7mm/行） | 排版美观，不填满也不违规 |
| 左右边距 2.5cm，上下边距 2.5cm | 标准 Word 排版 |
...
|---|---|
| 纸张 A4 | 《登记办法》 |
| 页眉软件全称 + 版本号，与源代码一致 | 版权中心 |
| 页码连续 | 版权中心 |
| 每个核心功能至少 1 张截图 | SSIA 申报指南 |
| 截图中软件名称、版本号与申请表一致 | SSIA |
| 主要功能描述 500-1300 字 | 2026 版权中心新规 |
| 功能描述基于真实项目，不编造 | 2026 新政实质性审查 |

#### 质量建议（不强制，但做了更好）

| 建议 | 理由 |
|---|---|
| 正文宋体，标题字体加粗区分层级 | 阅读体验好 |
| 目录用 Word 自动目录（TOC 域代码） | 页码自动更新，手写也可以 |
| 页眉页码用 Word PAGE/NUMPAGES 域代码 | 自动更新，手动写也不违规 |
| 一级标题用中文大写（一、二、三、） | 结构清晰，阿拉伯数字也可以 |
| 正文自然段落，避免过多列表 | 更像正式手册 |
| 截图预留文字用 `【截图预留：...】` | 提醒用户补充截图 |
