...
**目录处理**
- 正文所有标题写完后，插入 TOC 域代码：`TOC \o "1-3" \h \z \u`
- 域代码 result 区不填内容，或只写一句"（请在 Word 中更新域）"
- 设置 `w:updateFields = true`，Word 打开时自动提示更新
- 用户打开文档后点击"是"或右键→更新域，Word 自动扫描标题生成目录和页码
```

**注**：不要"文档修改记录"——最终提交的文档不需要这个。分页使用 XML 级别的 `<w:br w:type="page"/>`，而不是 python-docx 的 `add_page_break()`（旧版本可能插入的是普通换行符）。

**写作要求**
- 正文自然段落，少用列表
- 禁止"进入方式："、"操作步骤："等字段模板
- **每个页面必须详细描写**：页面用途、进入位置、用户看到的控件和数据、用户可执行的操作、操作的反馈结果、截图预留。不要写一两句就结束
- 语言面向普通用户，避免技术术语
- 每个截图位置输出两行：
  第一行（占位）：`【截图：页面名称 - 显示内容描述】`，用户替换为实际图片后删除此行
  第二行（图注）：页面名称，如`用户注册界面`，保留不动
- 手机应用用竖版占位（宽4.5cm×高8cm），桌面应用用横版，避免插入图片后布局大变
