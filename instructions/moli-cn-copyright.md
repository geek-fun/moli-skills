...
**写作要求**
- 每个功能必须详细描写：命名具体 UI 区域（用引号标注）、分步操作（1. 2. 3.）、前置条件、操作反馈
- 每个章节结构：功能简介（一段概述）→ 使用说明（逐步骤操作）→ 截图占位
- 步骤中的 UI 控件用引号标注，如"手机号输入框""拍照按钮""确认删除"
- 每个步骤描述用户动作和系统反馈：点击什么 → 看到什么
- 正文自然段落，功能概述用自然段落，操作步骤用编号 1. 2. 3.
- 禁止"进入方式："、"操作步骤："等字段模板
- 语言面向普通用户，避免技术术语
- 截图占位三步格式：虚线边框表格（模拟截图区域）+ 可删除占位行 `【截图：页面名称 - 描述】` + 永久居中图注（页面名称）
- 手机应用用竖版（宽4.5cm×高8cm），桌面应用用横版
- **页数要求**：内容必须足够详细，自然达到 15 页以上（不依赖截图凑页数，截图只是补充说明）
...
- 语言面向普通用户，避免技术术语
- 每个截图位置输出三部分，全部居中：
  1. 虚线边框表格：模拟截图区域，标明"【截图区域】"和应用类型尺寸
  2. 占位行（可删除）：`【截图：页面名称 - 显示内容描述】`
  3. 图注行（永久保留）：页面名称，如`用户注册界面`
- 手机应用用竖版（宽4.5cm×高8cm），桌面应用用横版
