...
- 字体：宋体（Songti/SimSun）或等宽字体（Courier New/Consolas）
- 字号：五号（10.5pt）
- 行间距：单倍行距（1.0），段间距 0 磅
- 颜色：黑色（禁止彩色）
- 编码：UTF-8

**页眉页脚**
- 页眉左侧：软件全称 + 版本号（如"山野集-菌迹软件 V1.0"）
- 页眉右侧：第 X 页（页码）
- 页眉底线：有分隔线
- 禁止：页眉出现公司名称、URL、文档类型字样（不写"源代码"）

**内容**
- 总页数：正好 60 页（前 30 + 后 30），不足 60 页全部提交
- 每页：≥ 50 行有效代码，无空行，无注释
- 最后一页：以完整模块结束（如 `};`），最后页≥15行
- 代码必须来自真实项目，禁止 AI 编造
- 剔除遗留代码（mockData、模板文件、编译产物）
...
- 正文字体：宋体，字号 小四 (12pt) 或 五号 (10.5pt)
- 正文行距：1.5 倍行距
- 段落缩进：首行缩进 2 字符
- 标题字体：黑体（Heiti/SimHei）
- 一级标题：黑体 三号(16pt) 加粗
- 二级标题：黑体 四号(14pt) 加粗
- 页眉：与源代码页眉一致（软件全称 + 版本号）
- 页码：连续，从1开始
